...
### Convex Sets
A set $\mathcal{X}$ is convex if the line segment connecting any two points within the set also lies entirely within the set. 
Mathematically, if $x' \in \mathcal{X}$ and $x'' \in \mathcal{X}$, then for all $0 \le t \le 1$:
...
If a function $g(x)$ is convex, then its sublevel set $\{g(x) \le c\}$ is also a convex set. 
*   **Proof:** If $x', x'' \in \{g(x) \le c\}$, we must prove $g(tx' + (1-t)x'') \le c$. Because of the convexity of $g(\cdot)$:
    
    $$
    g(tx' + (1-t)x'') \le tg(x') + (1-t)g(x'') \le tc + (1-t)c = c
    $$
...
*   **Balls:** $\{x \mid \|x - c\| < r\}$
*   **Polytopes:** Defined as the intersection of multiple hyperplanes. Represented as $\{x \mid Ax \le b\}$ or $\{x \mid a_i^\top x \le b \quad \forall i = 1, \dots, r\}$.

### Supporting Hyperplanes
**Theorem:** If a set is convex, there exists a supporting hyperplane at every point $z$ on its boundary.
If the set is defined as $\{x \mid g(x) \le 0\}$, the supporting hyperplane at $z$ is:
...
*   **Norms:** $f(x) = \|x - c\|$
*   **Quadratic functions:** $f(x) = x^\top Ax + b^\top x$ (requires $A \ge 0$, meaning $A$ must be positive semi-definite).
...
*   **Proof:** Suppose $x^*$ is locally optimal, but there exists a $y$ such that $f(y) < f(x^*)$. Let $z = ty + (1-t)x^*$ with $t = \frac{R}{2\|y - x^*\|}$. By convexity, $z$ is feasible and inside a ball of radius $R$. However, $f(z) \le tf(y) + (1-t)f(x^*) < f(x^*)$, which contradicts the local optimality assumption.

### Local Optimality Conditions (Differentiable Case)
**Proposition:** $x^*$ is a local optimum for a differentiable problem if and only if:
...
When constraints are present (e.g., $g(x) \le 0$), the unconstrained optimum $\nabla f(x) = 0$ might lie outside the feasible set. To find the optimal solution on the boundary, we use the **Karush-Kuhn-Tucker (KKT)** conditions.
